"""TD Bank Business bank statement parser."""
...
class TDTemplate(BankTemplate):
    ...
    def matches(self, text: str, tables: list[ExtractedTable]) -> float:
        ...
        if "TD BANK" in text_upper or "TD BUSINESS" in text_upper:
            score += 0.4
        if "tdbank.com" in text_lower:
            score += 0.2
        if "TD BUSINESS CONVENIENCE" in text_upper or "TD BUSINESS PREMIER" in text_upper:
            score += 0.2
        if "Other Credits" in text or "Other Debits" in text:
            score += 0.1
    ...
    def extract_summary(self, text: str, tables: list[ExtractedTable]) -> BankStatementSummary:
        summary = BankStatementSummary(bank_name="TD Bank", template_used="td")
...
TemplateRegistry.register(TDTemplate())
