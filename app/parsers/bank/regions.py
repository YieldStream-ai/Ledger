"""Regions Bank Business bank statement parser."""
...
class RegionsTemplate(BankTemplate):
    ...
    def matches(self, text: str, tables: list[ExtractedTable]) -> float:
        ...
        if "REGIONS BANK" in text_upper or "REGIONS FINANCIAL" in text_upper:
            score += 0.4
        if "regions.com" in text_lower:
            score += 0.2
        if "Business Checking" in text and "Regions" in text:
            score += 0.2
        if "LifeGreen" in text:
            score += 0.1

        return min(score, 1.0)

    def extract_summary(self, text: str, tables: list[ExtractedTable]) -> BankStatementSummary:
        summary = BankStatementSummary(bank_name="Regions", template_used="regions")
...
TemplateRegistry.register(RegionsTemplate())
