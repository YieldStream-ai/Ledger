"""BMO Harris Business bank statement parser."""
...
class BMOHarrisTemplate(BankTemplate):
    ...
    def matches(self, text: str, tables: list[ExtractedTable]) -> float:
        ...
        if "BMO HARRIS" in text_upper or "BMO BANK" in text_upper:
            score += 0.4
        if "bmoharris.com" in text_lower or "bmo.com" in text_lower:
            score += 0.2
        if "Business Checking" in text and "BMO" in text_upper:
            score += 0.2

        return min(score, 1.0)

    def extract_summary(self, text: str, tables: list[ExtractedTable]) -> BankStatementSummary:
        summary = BankStatementSummary(bank_name="BMO Harris", template_used="bmo_harris")
...
TemplateRegistry.register(BMOHarrisTemplate())
