"""Fifth Third Bank Business bank statement parser."""
...
class FifthThirdTemplate(BankTemplate):
    ...
    def matches(self, text: str, tables: list[ExtractedTable]) -> float:
        ...
        if "FIFTH THIRD" in text_upper:
            score += 0.4
        if "53.com" in text_lower or "fifththird.com" in text_lower:
            score += 0.2
        if "Business Checking" in text and "Fifth Third" in text:
            score += 0.2
        if "5/3" in text:
            score += 0.1
    ...
    def extract_summary(self, text: str, tables: list[ExtractedTable]) -> BankStatementSummary:
        summary = BankStatementSummary(bank_name="Fifth Third Bank", template_used="fifth_third")
        ...
        summary.total_withdrawals = find_amount(text, [
            r"Total\s+(?:Withdrawals|Debits|Checks)[:\s]*\$?([\d,]+\.?\d*)",
        ])
        summary.average_daily_balance = find_amount(text, [
            r"Average\s+(?:Daily\s+)?(?:Collected\s+)?Balance[:\s]*\$?([\d,]+\.?\d*)",
        ])
...
TemplateRegistry.register(FifthThirdTemplate())
